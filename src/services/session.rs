...
use crate::types::token::AuthorizeSessionPayload;
...
pub fn authorize(token: String, conn: &DBPool) -> Result<Session,&'static str> {
    JwtTokenService::validate::<AuthorizeSessionPayload>(token)
        .and_then(|r| SessionRepo::authorize(r.session_id, conn))
}

pub fn login(login_cred: LoginCred, conn: &DBPool) -> Result<Session, &'static str> {
